...
export function createPluginRoutes(_registry: ProviderRegistry): Hono {
...
export function createBatchRoutes(registry: ProviderRegistry): Hono {
  ...
  batchRoutes.post("/tts", async (c) => {
    try {
      ...
      const provider = body.provider ?? registry.autoSelectTTSProvider() ?? "gemini";
      const model =
        body.model ??
        (provider === "openai" ? "tts-1" : "gemini-2.5-flash-preview-tts");

      const generateFn = (request: any) => registry.generateTTS(request);
